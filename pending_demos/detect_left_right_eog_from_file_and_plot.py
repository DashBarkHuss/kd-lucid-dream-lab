"""
This script analyzes EOG (Electrooculography) data to detect and visualize left and right eye movements from a CSV file. 
It detects left eye movements as positive peaks above +400 μV and right eye movements as negative peaks below -400 μV. 
The script uses parameters like peak width, return width, and prominence to filter valid eye movements. 
The visualization is created using PyQtGraph and shows the raw EOG signal with color-coded markers for detected eye movements 
(red for left, green for right). The interactive plot includes a legend and text labels showing the position of each 
detected movement.
"""
