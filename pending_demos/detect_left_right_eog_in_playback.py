"""
This script analyzes EOG (electrooculography) data to detect and visualize left and right eye movements in real-time playback. 
It uses BrainFlow to read data from a CSV file and PyQtGraph for visualization. The script includes functions to detect eye 
movements based on signal peaks (positive peaks for left eye movements, negative peaks for right eye movements) with configurable 
thresholds. The visualization includes a real-time scrolling plot with green markers for left eye movements and red markers for 
right eye movements. The script can process both live streaming data and playback from recorded files, making it useful for both 
real-time monitoring and post-recording analysis.
"""
