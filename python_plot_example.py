"""
This script demonstrates real-time EEG data visualization using BrainFlow and PyQtGraph.
It connects to a Ganglion board via network streaming, processes the incoming EEG data
with various filters (detrending, bandpass, and bandstop), and displays the filtered
signals in a multi-channel time series plot. The Graph class handles the visualization
with auto-updating plots, while the main function manages the BrainFlow board connection.
This example is useful for monitoring brain activity in real-time with noise reduction
and can serve as a starting point for more complex BrainFlow-based applications.
"""
